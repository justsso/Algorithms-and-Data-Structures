import Link from "next/link";

export default function LeetCodeList({posts}) {
    return (
        <ul className="list-disc font-serif">
            {posts.map((item, index) => {
                const JSsuffix = item.slug.endsWith('.js')
                const MDsuffix = item.slug.endsWith('.md')
                return <li key={index} className="mb-6 antialiased hover:underline  cursor-pointer">
                    <Link href="/leetcode/[slug]" as={`leetcode/${item.slug}`}>
                        <span>
                            {JSsuffix && (
                                <svg t="1589873091973" className="icon stroke-current inline-block h-4 w-4 mr-2"
                                     viewBox="0 0 1024 1024" version="1.1"
                                     xmlns="http://www.w3.org/2000/svg" p-id="3358" width="64" height="64">
                                    <path
                                        d="M897.4 63.9H128.2c-34.9 0-63.5 28.6-63.5 63.5v769.2c0 34.9 28.6 63.5 63.5 63.5h769.2c34.9 0 63.5-28.6 63.5-63.5V127.4c0-34.9-28.6-63.5-63.5-63.5z m-0.5 768.3c0 34.9-28.6 63.5-63.5 63.5H192.2c-34.9 0-63.5-28.6-63.5-63.5V191.4c0-34.9 28.6-63.5 63.5-63.5h641.3c34.9 0 63.5 28.6 63.5 63.5v640.8z"
                                        fill="#F5AF3F" p-id="3359"/>
                                    <path
                                        d="M477.2 555.4c0 56-13.6 99-40.7 128.9-27.1 29.9-66 44.9-116.6 44.9-22.4 0-43.2-3.7-62.4-10.9v-85.1c16.5 11.9 35.3 17.8 56.4 17.8 45.6 0 68.4-32.9 68.4-98.8V297.8h94.9v257.6z m51.7 85.1c31.8 18.4 63 27.6 93.5 27.6 38.3 0 57.5-9.8 57.5-29.3 0-13.9-15.8-25.5-47.5-34.8-39.5-11.7-66.6-24.7-81.4-39-14.8-14.3-22.1-33.7-22.1-58.2 0-29.7 12.7-53.1 38-69.9 25.3-16.9 58.5-25.3 99.5-25.3 29.1 0 57.2 4.1 84.3 12.3v69.5c-24.7-13.9-52-20.8-81.7-20.8-14.8 0-26.7 2.5-35.7 7.5-9 5-13.5 11.6-13.5 19.8 0 13.9 13.4 25.1 40.3 33.7 28.8 9.1 50.4 17.4 64.9 24.9 14.5 7.5 25.5 17.3 33.1 29.6 7.6 12.2 11.4 26.4 11.4 42.4 0 31.2-13.1 55.4-39.4 72.7-26.3 17.2-61.4 25.9-105.3 25.9-34.5 0-66.4-5.2-95.8-15.6v-73z"
                                        fill="#F5AF3F" p-id="3360"/>
                                </svg>)}
                            {MDsuffix && (
                                <svg t="1589873347688" className="icon inline-block h-4 w-4 mr-2"
                                     viewBox="0 0 1024 1024" version="1.1"
                                     xmlns="http://www.w3.org/2000/svg" p-id="7080" width="32" height="32">
                                    <path d="M622.078 64.636l216.055 216.057H622.078V64.636z" fill="#515151"
                                          p-id="7081"/>
                                    <path
                                        d="M588.763 64.636V309.01h244.332v650.353H183.387V64.636h405.376z m-361.33 362.218v484.581h561.614V426.85l-561.615 0.004z m99.572 387.843V524.714H399.5l108.74 108.748 108.746-108.748h72.49v289.983h-72.49V627.299L508.24 736.039 399.5 627.3v187.398h-72.496"
                                        fill="#515151" p-id="7082"/>
                                </svg>
                            )}
                            <a className="text-lg">{item.slug}</a>
                        </span>
                    </Link>
                </li>
            })}
        </ul>
    )
}
