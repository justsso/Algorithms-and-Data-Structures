...
foo(1,2,3).getValue()   // 6

foo(1)(2,3).getValue()  // 6

foo(1)(2)(3)(4).getValue()  // 10
...
/*
* 思路：
  foo函数可以接收一个到多个参数，也可以连着调用()，最后调用返回值的getValue()返回所有参数的和
* 所以我们每次调用foo()，就处理入参，将新传入的参数整合到以往的入参中，最后返回一个闭包(函数)，这个闭包还有一个getValue()，当调用getValue()时，再去计算所有的传入的参数
 */
